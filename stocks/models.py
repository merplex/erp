...
class PurchaseOrder(models.Model):
    ...
    status = models.CharField(max_length=20, default='Draft', choices=[('Draft','ร่าง'),('Confirmed','ยืนยัน'),('Received','รับของแล้ว')])
...
class PurchaseItem(models.Model):
    ...
    quantity_received = models.PositiveIntegerField(default=0, verbose_name="จำนวนที่รับจริง")
    def __str__(self): return f"{self.product.name} ({self.quantity_received}/{self.quantity_ordered})"
...
class SalesOrder(models.Model):
    ...
    order_date = models.DateField(default=datetime.date.today)
    status = models.CharField(max_length=20, default='Draft', choices=[('Draft','ร่าง'),('Confirmed','ยืนยัน'),('Shipped','ส่งของแล้ว')])
    notes = models.TextField(blank=True, verbose_name="หมายเหตุ")
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    def save(self, *args, **kwargs):
...
class SalesItem(models.Model):
    ...
    quantity_shipped = models.PositiveIntegerField(default=0, verbose_name="จำนวนที่ส่งจริง")
...
class ProductionOrder(models.Model):
    ...
    quantity_actual = models.PositiveIntegerField(default=0, verbose_name="จำนวนที่ผลิตจริง")
    order_date = models.DateField(default=datetime.date.today)
    status = models.CharField(max_length=20, default='Draft', choices=[('Draft','ร่าง'),('Started','เริ่มผลิต'),('Finished','ผลิตเสร็จ')])
    notes = models.TextField(blank=True, verbose_name="หมายเหตุ")
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    def save(self, *args, **kwargs):
